...
|----------|------------|-----------|
| k3s-master-1 | 172.31.94.38 | 52.91.103.21|
| k3s-master-2 | 172.31.89.179 | 3.89.140.36 |
| k3s-master-3 | 172.31.90.248 | 52.207.220.176 |
...
During this assignment, I gained hands-on experience setting up a lightweight Kubernetes cluster (K3s) in an AWS environment, which provided valuable insights into container orchestration and cloud-native deployment practices. One of the key learnings was understanding how K3s simplifies the installation and management of Kubernetes, particularly for small-scale or edge deployments, while maintaining compatibility with standard Kubernetes APIs and resources. Through deploying pods, verifying node readiness, and configuring an Ingress Controller, I learned how workloads are scheduled across nodes and how services can be exposed externally.

Several challenges arose during the process. Initially, I faced SSH connectivity issues to the AWS EC2 instances, resulting in “Connection timed out” errors. Resolving this required reviewing and adjusting AWS security group rules to allow inbound SSH (port 22) traffic and ensuring the correct key pair was used for authentication. Another challenge was copying the K3s join token correctly between nodes; an extra space or line break would prevent the secondary master or worker nodes from joining the cluster. Careful attention to formatting and verifying the token resolved this. Additionally, configuring the /etc/hosts file properly was essential for node-to-node communication and cluster stability.

This assignment highlighted the relationship between K3s and production Kubernetes environments, particularly in the context of cloud-native concepts such as microservices, orchestration, and 5G edge deployments. K3s provides a lightweight alternative for scenarios where full Kubernetes would be too resource-intensive, demonstrating the flexibility needed in modern cloud and 5G infrastructures, where low-latency and distributed deployments are critical. Understanding this helped me appreciate how production-grade Kubernetes clusters manage scalability, high availability, and service discovery at a larger scale.

Finally, the exercise reinforced how virtualization and containerization work together to enable scalable and efficient services. Virtual machines provide isolated environments for nodes, while containers allow applications to run consistently across different environments without dependency conflicts. This combination allows workloads to be rapidly deployed, scaled, or moved between nodes, which is fundamental in cloud-native architectures. By working with K3s, I experienced firsthand how container orchestration ensures reliability and scalability, even on a small cluster, which mirrors the principles used in large-scale production and 5G-ready cloud platforms.

Overall, this assignment not only strengthened my practical skills in Kubernetes and AWS but also deepened my understanding of the concepts behind containerized, scalable cloud services and their role in modern IT infrastructures.
